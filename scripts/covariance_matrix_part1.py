...
AbacusSummit data, runs the usual density split and power spectrum data, and
saves to file (to later be used to calculate covariance matrices).
"""
            ...
            for split in ['z', 'r']:
                ...
                for i in range(len(rand_position_list)):
                    ...
                    auto_result = CatalogFFTPower(data_positions1=rand_positions,
                                             ...
                                             los=los, position_type='pos', ells=[0])
                    ...
                    pickle.dump(auto_result, open(
                        os.path.join(power_path, 'Auto_{0}_{1}_sim{2}.pkl'.format(split, density_label, sim_num)) , 'wb'))
                    pickle.dump(cross_result, open(
                        os.path.join(power_path, 'Cross_{0}_{1}_sim{2}.pkl'.format(split, density_label, sim_num)), 'wb'))
                
        else: # skips if simulation failed and thus no path exists
            continue

    print('Power spectra generated in: ', round(time.time()-t0, 1) , 'seconds.')